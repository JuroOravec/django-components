...
2. BREAKING: Rework dependencies middleware.
  - Add option to inline into the HTML the string literal ("inlined") JS / CSS
  - NOTE: Would make more sense to release it together with the client manager,
    so we can also introduce per-component execution, so people understand,
...
#        - Would be also used for validating settings
...
# TODO - `js_name` and `css_name`
# TODO - Allow to recursively pass typed tuples and dicts
...
#    - AUTOMATICALLY INCLUDE {% load component_tags %} in inlined HTML?
...
#    - maybe rename `html_attrs` to `attrs`?
