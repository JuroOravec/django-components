---
title: Autodiscovery
url: https://jurooravec.github.io/django-components/docs/concepts/fundamentals/autodiscovery/
description: "django-components automatically searches for files containing components in the
COMPONENTS.dirs and 
COMPONENTS.app_dirs
directories."
---
...
[`COMPONENTS.dirs`][ComponentsSettings.dirs] and 
...
[`{% component %}`](../../reference/template_tags.md#component)
tag needs to be registered with the [`ComponentRegistry`][ComponentRegistry].

We use the [`@register`][register] decorator for that:
...
- Components inside the auto-imported files still need to be registered with [`@register`][register]
...
Autodiscovery can be disabled in the settings with [`autodiscover=False`][ComponentsSettings.autodiscover].

## Manually trigger autodiscovery

Autodiscovery can be also triggered manually, using the [`autodiscover()`][autodiscover] function. This is useful if you want to run autodiscovery at a custom point of the lifecycle:
...
To get the same list of modules that [`autodiscover()`][autodiscover] would return,
but without importing them, use [`get_component_files()`][get_component_files]:
