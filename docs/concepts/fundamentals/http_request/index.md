---
title: HTTP Request
url: https://jurooravec.github.io/django-components/docs/concepts/fundamentals/http_request/
description: "The most common use of django-components is to render HTML when the server receives a request. As such,
there are a few features that are dependent on..."
---
...
explicitly to [`Component.render()`][Component.render] and
...
- The component is rendered with a regular [`Context`](https://docs.djangoproject.com/en/5.2/ref/templates/api/#django.template.Context) (or none), but you set the `request` kwarg
    of [`Component.render()`][Component.render].
...
When the component has access to the `request` object, the request object will be available in [`Component.request`][Component.request].
...
You can also access the context processors data from within [`get_template_data()`][Component.get_template_data] and other methods under [`Component.context_processors_data`][Component.context_processors_data].
...
!!! warning
    ...
    The [`self.context_processors_data`][Component.context_processors_data] object is generated dynamically, so changes to it are not persisted.