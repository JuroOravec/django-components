...
url: https://jurooravec.github.io/django-components/docs/concepts/fundamentals/typing_and_validation/
description: "In versions 0.92 to 0.139 (inclusive), the component typing was specified through generics."
---
...
The [`Component`][Component] class optionally accepts class attributes
...
- [`Args`][Component.Args] - Type for positional arguments.
- [`Kwargs`][Component.Kwargs] - Type for keyword arguments.
- [`Slots`][Component.Slots] - Type for slots.
- [`TemplateData`][Component.TemplateData] - Type for data returned from [`get_template_data()`][Component.get_template_data].
- [`JsData`][Component.JsData] - Type for data returned from [`get_js_data()`][Component.get_js_data].
- [`CssData`][Component.CssData] - Type for data returned from [`get_css_data()`][Component.get_css_data].

You can specify as many or as few of these as you want, the rest will default to `None`.

## Typing inputs

You can use [`Component.Args`][Component.Args],
[`Component.Kwargs`][Component.Kwargs],
and [`Component.Slots`][Component.Slots] to type the component inputs.
...
If you omit the [`Args`][Component.Args],
[`Kwargs`][Component.Kwargs], or
[`Slots`][Component.Slots] classes,
...
!!! info
    ...
    Components can receive slots as strings, functions, or instances of [`Slot`][Slot].

    Internally these are all normalized to instances of [`Slot`][Slot].

    Therefore, the `slots` dictionary available in data methods (like
    [`get_template_data()`][Component.get_template_data])
    will always be a dictionary of [`Slot`][Slot] instances.

    To correctly type this dictionary, you should set the fields of `Slots` to
    [`Slot`][Slot] or [`SlotInput`][SlotInput]:

    [`SlotInput`][SlotInput] is a union of `Slot`, string, and function types.

## Typing data

You can use [`Component.TemplateData`][Component.TemplateData],
[`Component.JsData`][Component.JsData],
and [`Component.CssData`][Component.CssData] to type the data returned from [`get_template_data()`][Component.get_template_data], [`get_js_data()`][Component.get_js_data], and [`get_css_data()`][Component.get_css_data].
...
If you omit the [`TemplateData`][Component.TemplateData],
[`JsData`][Component.JsData], or
[`CssData`][Component.CssData] classes,
...
[`TemplateData`][Component.TemplateData],
...
This can get repetitive, so we added a [`Empty`][Empty] type to make it easier:
...
Since v0.140, the types must be specified as class attributes of the [Component][Component] class -
[`Args`][Component.Args],
[`Kwargs`][Component.Kwargs],
[`Slots`][Component.Slots],
[`TemplateData`][Component.TemplateData],
...
To type-check the inputs, you should wrap the inputs in [`Component.Args`][Component.Args],
[`Component.Kwargs`][Component.Kwargs],
[`Component.Slots`][Component.Slots], etc.
...
3. If you defined typing for the data methods (like [`get_context_data()`][Component.get_context_data]), move them inside the Component class, and rename them to `TemplateData`, `JsData`, and `CssData`.
4. Remove the `Component` generic.
5. If you accessed the `args`, `kwargs`, or `slots` attributes via
    [`self.input`][Component.input], you will need to add the type hints yourself, because [`self.input`][Component.input] is no longer typed.

    Otherwise, you may use [`Component.get_template_data()`][Component.get_template_data] instead of [`get_context_data()`][Component.get_context_data], as `get_template_data()` receives `args`, `kwargs`, `slots` and `context` as arguments. You will still need to add the type hints yourself.

6. Lastly, you will need to update the [`Component.render()`][Component.render]
    calls to wrap the inputs in [`Component.Args`][Component.Args], [`Component.Kwargs`][Component.Kwargs], and [`Component.Slots`][Component.Slots], to manually add type hints.
