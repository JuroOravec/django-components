...
url: https://jurooravec.github.io/django-components/docs/concepts/advanced/component_libraries/
description: "You can publish and share your components for others to use. Below you will find the steps to do so."
---

You can publish and share your components for others to use. Below you will find the steps to do so.

For live examples, see the [Examples](../../../../examples/).
...
2.  Create custom [`Library`](https://docs.djangoproject.com/en/5.2/howto/custom-template-tags/#how-to-create-custom-template-tags-and-filters)
    and [`ComponentRegistry`][ComponentRegistry] instances in `mytags.py`
    ...
    using the [`settings`][ComponentRegistry.settings] argument.
    If omitted, default settings are used.

    For library authors, we recommend setting [`context_behavior`][ComponentsSettings.context_behavior]
    to [`"isolated"`][ContextBehavior.ISOLATED], so that the state cannot leak into the components,
    ...
    [`tag_formatter`][ComponentsSettings.tag_formatter]
    options.
...
3.  Write your components and register them with your instance of [`ComponentRegistry`][ComponentRegistry]
    ...
    [`ComponentRegistry`][ComponentRegistry] from the previous step.
...
4.  Import the components in `apps.py`
    ...
    Normally, users rely on [autodiscovery](../fundamentals/autodiscovery.md) and [`COMPONENTS.dirs`][ComponentsSettings.dirs]
