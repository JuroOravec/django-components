...
url: https://jurooravec.github.io/django-components/docs/getting_started/rendering_components/
description: "Our calendar component can accept and pre-process data, defines its own CSS and JS, and can be used in templates."
---
...
- Render the component directly with [`Component.render()`][Component.render]
- Render the component directly with [`Component.render_to_response()`][Component.render_to_response]
...
You can also render the component directly with [`Component.render()`][Component.render], without wrapping the component in a template.
...
For this, you can use the [`Component.render_to_response()`][Component.render_to_response] convenience method.

`render_to_response()` accepts the same args, kwargs, slots, and more, as [`Component.render()`][Component.render], but wraps the result in an [`HttpResponse`](https://docs.djangoproject.com/en/5.2/ref/request-response/#django.http.HttpResponse).
...
!!! info "Response class of `render_to_response`"
    ...
    If you want to use a different Response class in `render_to_response`, set the [`Component.response_class`][Component.response_class] attribute:
...
You can define the view endpoints directly on your component class by using the nested [`Component.View`][Component.View] class.

This is a great place for:

- Web pages - Page components can use [`Component.View.get()`][ComponentView.get] to return the entire web page upon GET request.

- Fragments - Fragment components (partials) can use [`Component.View.get()`][ComponentView.get] to return HTML fragments upon GET request (to be used with HTMX or similar libraries).

- Form submission - Define [`Component.View.post()`][ComponentView.post] to handle a POST requests.
...
1. Use automatically generated component URL using [`get_component_url()`][get_component_url]:
    ...
    To explicitly enable/disable the component's URL, use [`Component.View.public`][ComponentView.public]:
...
2. Manually assign the URL by setting [`Component.as_view()`][Component.as_view] to your `urlpatterns`:
...
!!! info "Customizing component URLs"
    ...
    You can customize the auto-generated component URL path by overriding [`Component.View.get_route_path()`][ComponentView.get_route_path].
