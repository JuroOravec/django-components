...
# # - Components have no side-effects
# # - What components render varies only with the input
# # - All template tags are deterministic - They return the same output for the same input
# # - The component's Template does NOT change
# #
# #
# # OPTIMIZATION LEVELS:
# # 0. No optimization done
# # 1. If component inputs are ALL constants, then we assume that the component is a constant
# #    and we can replace it with a string that the component has rendered.
# #    - In practice, the component node will still be there, but its Template's Nodelist
# #      will be only the rendered string. And maybe we can skip over some checks.
# #    - Tho slot nodes are preserved, unless 1) the slot is ALSO marked as constant.
# #      User can either manually mark slot as constant, or if it's a `{% fill %}`,
# #      we check if all variables within it are constant.
# # 2. Only SOME component inputs are constants. We replace those Nodes within Component's nodelist
# #    that are constant.
# #
# # NOTES:
# # - If which slots are given does not change, then `component_vars.is_filled` could also become a constant.
# #    Thus also empty slots could be removed or replaced with their default.
# # - We should control `{% for %}` and `{% if %}` nodes, so we can replace them with their rendered output
# #   if they are constant.
# # - `constant_map` would run only on first render, it shouldn't be needed afterwards.
# # - To actually reliably define a specific use of a component, we'd have to include FULL component tree
# #   path, including the index within the closest template tag.
# #   - NOTE: Though, this then means that we might not be able to support all Django's template tags?
# #     - At least, we will have to review them one by one.
# #     - And so this is definitely feasible only once at v2.
# # - In `{% for %}` loop, if the whole list is not const, but if there are some constant items
# #   in the list, and the "fill" is constant, then those respective list items could be replaced with
# #   a string.
...
#     def render():
#         nonlocal result
#         result = ProjectPage.render_to_response(kwargs=data)

#     ITERATIONS = 1
#     django_debug_time = timeit.timeit(lambda: render(), number=ITERATIONS)
#     print(f"Render time: {django_debug_time/ITERATIONS:.6f} seconds")
...
#         else:
#             button_classes = get_styling_css(variant, "default", disabled)  # type: ignore[arg-type]
#             all_css_class = (
#                 f"{ button_classes } { common_css } px-3 py-2 justify-center rounded-md shadow-sm"
#             )

#         is_link = not disabled and href
...
#                 x-anchor.bottom="contextMenuRef.value"
#                 x-show="contextMenuItem.value"
#                 x-cloak=""
#                 class="w-24 ml-8 z-40 mt-2 divide-y divide-gray-300 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
#             >
...
#                         tabindex="0"
#                         href="#"
#                         class="px-4 py-1 text-sm text-gray-900 hover:bg-gray-100 cursor-pointer block"
...
#             link="#",
#             item_attrs={
#                 "class": "px-4 py-1 text-sm text-gray-900 hover:bg-gray-100 cursor-pointer",
#                 ":href": "contextMenuItem.value.edit_url",
#             },
#         )]
#     ]
...
#     template: types.django_html = """
#         <div @user_delete="onUserDelete" class="flow-root">
...
#                     <svg viewBox="0 0 24 24" style="width: 24px; height: 24px" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.5" class="inline hover:bg-neutral-700 hover:text-white transition h-6 w-6 shrink-0">
...
#                 {% component "TabsStatic" tabs=top_level_tabs index=1 / %}
...
#                 id: $id('modal-title'),
#             }"
#             @keydown.escape="isDeleteDialogOpen = false"
#         >
#             <div
#                 class="relative z-50"
#                 :aria-labelledby="id"
...
#                             @click.away="isDeleteDialogOpen = false"
#                         >
...
#                                 <button type="submit" href="#" :href="role.delete_url" @click="isDeleteDialogOpen = false;" class="bg-white text-gray-800 ring-gray-300 hover:bg-gray-100 focus-visible:outline-gray-600 transition ring-1 ring-inset inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm" style="color: cornflowerblue;">
#                                     Delete
#                                 </button>
#                                 <button type="submit" href="#" :href="role.delete_url" @click="isDeleteDialogOpen = false;" class="bg-white text-gray-800 ring-gray-300 hover:bg-gray-100 focus-visible:outline-gray-600 transition ring-1 ring-inset inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm" style="color: cornflowerblue;">
#                                     Cancel
...
#                         {% if editable %}
#                             <div>
#                                 <button @click="removeTag(index)" class="!py-1 bg-white text-gray-800 ring-gray-300 hover:bg-gray-100 focus-visible:outline-gray-600 transition ring-1 ring-inset inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm" style="color: cornflowerblue;">
#                                     Remove
#                                 </button>
#                             </div>
#                         {% endif %}
...
#                     <button @click="addTag" class="!py-1 bg-white text-gray-800 ring-gray-300 hover:bg-gray-100 focus-visible:outline-gray-600 transition ring-1 ring-inset inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm" style="color: cornflowerblue;">
#                         Add tag
#                     </button>
#                 </div>
#             {% endif %}
...
#     template: types.django_html = """
#         <form
#             {% if submit_href %} action="{{ submit_href }}" {% endif %}
#             method="post"
#             x-data="form"
#         >
#             <div
...
#     template: types.django_html = """
#         <li x-data="bookmarks" class="pt-4">
#             <svg viewBox="0 0 24 24" style="width: 24px; height: 24px" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.5" class="inline hover:bg-neutral-700 hover:text-white transition h-6 w-6 shrink-0">
#                 <path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5"></path>
#                 Project Bookmarks
#             </svg>
...
#             {% for item in items %}
#                 <li {% html_attrs item.attrs class="py-5 group flex justify-between gap-x-6 border border-gray-300 pl-4 pr-6 bg-white" %}>
...
#                     {% if not tab.disabled %}
#                         <li {% html_attrs class="border-b-2" class=styling.tab %}>
#                             <a
#                                 href="{{ tab.href }}"
#                                 {% html_attrs
#                                     class="bg-white inline-block py-2 px-4 font-semibold transition"
...
#     formatted_modified_by = (
#         f"{modified_time_str}\n"
#         f"{status_update['modified_by']['name']}"
#     )
...
#                             <button type="button" x-bind:href="attachment.url" x-text="attachment.text" target="_blank" class="hover:text-gray-600 !underline !py-1 bg-white text-gray-800 ring-gray-300 hover:bg-gray-100 focus-visible:outline-gray-600 transition ring-1 ring-inset inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm" style="color: cornflowerblue;">
#                             </button>
#                         </div>
...
#                                 <div>
#                                     <button type="button" x-text="attachment.isPreview ? 'Edit' : 'Preview'" @click="() => $emit('toggleAttachment', index)" class="!py-1 bg-white text-gray-800 ring-gray-300 hover:bg-gray-100 focus-visible:outline-gray-600 transition ring-1 ring-inset inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm">
#                                         Edit
#                                     </button>
#                                 </div>
                                
#                                 <div>
#                                     <button type="button" @click="() => $emit('removeAttachment', index)" class="!py-1 bg-red-600 text-white hover:bg-red-500 focus-visible:outline-red-600 transition inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm">
#                                         Remove
#                                     </button>
#                                 </div>
#                             </div>
...
#                         {% if editable %}
#                             <button type="button" @click="addAttachment" class="bg-white text-gray-800 ring-gray-300 hover:bg-gray-100 focus-visible:outline-gray-600 transition ring-1 ring-inset inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm">
#                                 Add attachment
#                             </button>

#                             <button type="button" @click="onOutputSubmit({ reload: true })" class="bg-blue-600 text-white hover:bg-blue-500 focus-visible:outline-blue-600 transition inline-flex w-full text-sm font-semibold sm:mt-0 sm:w-auto focus-visible:outline-2 focus-visible:outline-offset-2 px-3 py-2 justify-center rounded-md shadow-sm">
#                                 Save
#                             </button>
#                         {% endif %}
#                     </div>
...
# # TODO
# # with open("test_djc.html", "wb") as f:
# #     f.write(project_view().content)
