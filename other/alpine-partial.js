...
(() => {
  ...
  // TODO
  let prepareMagicProp = (prop) => {
    const magicName =
      typeof prop === "string" && prop.startsWith("$") && prop.slice(1);
    return magicName || null;
  };
  window.injectMagicsCalls = 0;
  window.injectMagicsEls = new Set();
  /** @type {Map<HTMLElement, MagicUtils} */
  let cachedUtils = new Map();

  // TODO
  function injectMagics(obj, el) {
    // window.injectMagicsCalls++;
    // window.injectMagicsEls.add(el);
    ...
    const magicProxy = new Proxy(obj, {
      get(target, prop) {
        const magicName = prepareMagicProp(prop);
      ...
      has(target, prop) {
        const magicName = prepareMagicProp(prop);
        const hasMagicHandler = !!(magicName && magics[magicName]);

        return hasMagicHandler || Reflect.has(target, prop);
      },
    });
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  function directives(el, attributes, originalAttributeOverride) {
    ...
    /** @type {Directive[]} */
    let directives2 = [];
    for (attr of attributesArr) {
      const transformedAttr = captureTransformedAttributes(attr);
      if (!transformedAttr.name.match(alpineAttrRegex)) continue;
      const directive = attrToDirective(transformedAttr);
      directives2.push(directive);
    }
    directives2 = directives2.sort(byPriority);
  ...
  function getIterationScopeVariables(iteratorNames, item, index, items) {
    let scopeVariables = {};
    if (iteratorNames.item.match(/^\[.*\]$/) && Array.isArray(item)) {
      let names = iteratorNames.item.replace("[", "").replace("]", "").split(",").map((i) => i.trim());
      names.forEach((name, i) => {
        scopeVariables[name] = item[i];
      });
    } else if (iteratorNames.item.match(/^\{.*\}$/) && !Array.isArray(item) && typeof item === "object") {
      let names = iteratorNames.item.replace("{", "").replace("}", "").split(",").map((i) => i.trim());
      names.forEach((name) => {
