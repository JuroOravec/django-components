...
(() => {
   ...
   * @param {string} prop 
   */
  let getMagicHandlerFromProp = (prop) => {
      const magicName = typeof prop === 'string' && prop.startsWith('$') && prop.slice(1);
      const magicHandler = magicName && magics[magicName];
      return magicHandler || null;
  }
  ...
  function injectMagics(obj, el) {
    ...
    const magicProxy = new Proxy(obj, {
        get(target, prop) {
            const magicHandler = getMagicHandlerFromProp(prop);
            if (magicHandler) {
                return magicHandler(el, {...utils});
            }
            return target[prop];
        },
        has(target, prop) {
            const magicHandler = getMagicHandlerFromProp(prop);;
            return !!magicHandler || Reflect.has(target, prop);
        },

        // ownKeys and getOwnPropertyDescriptor help with making the Alpine magics
        // visible in the devtools when inspecting the object that had magics injected.
        ownKeys(target) {
            // Convert e.g. `id` -> `$id` or `el` -> `$el`
            const magicProps = Object.keys(magics).map((key) => `$${key}`);
            return Reflect.ownKeys(target).concat(magicProps);
        },
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  function generateFunctionFromString(expression, el) {
    ...
    let rightSideSafeExpression =
      ...
      expression.trim().match(ifExpressionRegex) ||
      // Support expressions starting with "let/const" like: "let foo = 'bar'"
      expression.trim().match(letConstExpressionRegex)
        ? `(async()=>{ ${expression} })()`
        : expression;
  ...
  function directives(el, attributes, originalAttributeOverride) {
    ...
    const captureTransformedAttributes = toTransformedAttributes(
      (newName, oldName) => (transformedAttributeMap[newName] = oldName)
    );

    const alpineAttrRegex = alpineAttributeRegex();
    const attrToDirective = toParsedDirectives(
      alpineAttrRegex,
      transformedAttributeMap,
      originalAttributeOverride
    );

    const directives2 = attributesArr
      .map(captureTransformedAttributes)
      .filter(({ name }) => name.match(alpineAttrRegex))
      .map(attrToDirective)
      .sort(byPriority);
  ...
  function attributesOnly(attributes) {
    ...
    return Array.from(attributes)
      ...
      .filter((attr) => !attr.name.match(alpineAttrRegex));
  }
  // TODO END
  ...
  function deferHandlingDirectives(callback) {
    ...
    const flushHandlers = () => {
      ...
      //  stack[i]();  // no shift, just access by index
      //}
      stack.forEach((handler) => handler());
      // for (handler of stack) {
      // handler();
      // }
    };
    // TODO END
  ...
  function walk(el, callback) {
    ...
    // Selector to match any attribute that starts with `x-`
    ...
    /** @type {HTMLElement[]} */
    const skippedEls = [];
    let elWithAlpine = result.iterateNext();
    const nextEl = () => (elWithAlpine = result.iterateNext());
    while (elWithAlpine) {
      const isInsideSkipped = skippedEls.some((el) =>
        el.contains(elWithAlpine)
      );
      if (isInsideSkipped) {
        nextEl();
        continue;
  ...
  // TODO
  function initTree(el, walker = walk, intercept = null) {
  ...
  function dotSyntax(subject) {
    return subject.replace(dotRE, ".");
  }
  let camelRE = /-(\w)/g;
  function camelCase2(subject) {
    return subject.toLowerCase().replace(camelRE, (match, char) => char.toUpperCase());
  }
  function isNumeric(subject) {
    return !Array.isArray(subject) && !isNaN(subject);
  }
  let kebabWordBoundaryRE = /([a-z])([A-Z])/g;
  let kebabUnderscoreRE = /[_\s]/;
  function kebabCase2(subject) {
    if ([" ", "_"].includes(
      subject
    ))
      return subject;
    return subject.replace(kebabWordBoundaryRE, "$1-$2").replace(kebabUnderscoreRE, "-").toLowerCase();
  }
  ...
  // E.g. `{}` or `  {  }  `
  const emptyExpressionRegex = /^\s*\{\s*\}\s*$/;
  ...
  directive("data", (el, { expression }, { cleanup: cleanup2 }) => {
    ...
    // NOTE: Use `str.match()` instead of RegExp.test() because the latter is STATEFUL!
    const isEmptyExpression = !expression || expression === "{}" || expression.match(emptyExpressionRegex);
    // TODO
    let data2;
    if (!isEmptyExpression) {
      data2 = evaluate(el, expression, { scope: dataProviderContext });
    } else {
      data2 = {};
    }
    if (data2 === void 0 || data2 === true)
      data2 = {};
    data2 = injectMagics(data2, el);
  ...
  let iteratorArrayRE = /[_\s]/;
  let iteratorObjectRE = /^\{.*\}$/;
  function getIterationScopeVariables(iteratorNames, item, index, items) {
    let scopeVariables = {};
    if (iteratorNames.item.match(iteratorArrayRE) && Array.isArray(item)) {
      let names = iteratorNames.item.replace("[", "").replace("]", "").split(",").map((i) => i.trim());
      names.forEach((name, i) => {
        scopeVariables[name] = item[i];
      });
    } else if (iteratorNames.item.match(iteratorObjectRE) && !Array.isArray(item) && typeof item === "object") {
      let names = iteratorNames.item.replace("{", "").replace("}", "").split(",").map((i) => i.trim());
      names.forEach((name) => {
